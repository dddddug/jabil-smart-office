...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      SELECT o.*, u.real_name as employee_name, d.name as department_name
      FROM ${TABLE} o
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      INSERT INTO ${TABLE} (employee_id, overtime_date, start_time, end_time, hours, reason, type, status, created_at)
      VALUES ($1, $2, $3, $4, $5, $6, $7, 'PENDING', CURRENT_TIMESTAMP)
      RETURNING *
    `, [employeeId, overtimeDate, startTime, endTime, hours, reason, type || 'NORMAL']);
